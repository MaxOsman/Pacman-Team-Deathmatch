...
{
	if (cpuDirectionChangeTime[i] >= 240)
	{
		srand(time(0)+i);
...
void Pacman::RefreshPowerup(int i)
{
	...
	if (difference > 25.0f && _powerup[i]->isCollected == true)		//Powerup transparent
	{
		_powerup[i]->texture = _powerupTransTexture;
		powerupMessageState = 1;
	}
	if (difference > 30.0f && _powerup[i]->isCollected == true)		//Powerup Full
	{
...
void Pacman::Draw(int elapsedTime)
{
	...
	stream.str(string());
	stream << "Time Remaining: " << 100 - (globalTime / 60);
